...
export default function Mint() {
  ...
  // FOR MINTING
  const [how_many_wigzwaxtests, set_how_many_wigzwaxtests] = useState(1)

  const [wigzwaxtestsContract, setWigzwaxtestsContract] = useState(null)
  ...
  const [wigzwaxtestsPrice, setWigzwaxtestsPrice] = useState(0)
  ...
  async function signIn() {
    ...
    window.ethereum.enable()
      .then(function (accounts) {
        ...
        // checks if connected network is net (change this to rinkeby if you wanna test on testnet)
...
//
  ...
  async function callContractData(wallet) {
    ...
    const wigzwaxtestsContract = new window.web3.eth.Contract(ABI, ADDRESS)
    setWigzwaxtestsContract(wigzwaxtestsContract)

    const salebool = await wigzwaxtestsContract.methods.saleIsActive().call() 
    console.log("saleisActive" , salebool)
    setSaleStarted(salebool)

    const totalSupply = await wigzwaxtestsContract.methods.totalSupply().call() 
    setTotalSupply(totalSupply)

    const wigzwaxtestsPrice = await wigzwaxtestsContract.methods.wigzwaxtestsPrice().call() 
    setWigzwaxtestsPrice(wigzwaxtestsPrice)
   
  }
  
  async function mintWigzwaxtests(how_many_wigzwaxtests) {
    if (wigzwaxtestsContract) {
 
      const price = Number(wigzwaxtestsPrice)  * how_many_wigzwaxtests 

      const gasAmount = await wigzwaxtestsContract.methods.mintWigzWaxTests(how_many_wigzwaxtests).estimateGas({from: walletAddress, value: price})
      console.log("estimated gas",gasAmount)

      console.log({from: walletAddress, value: price})

      wigzwaxtestsContract.methods
            .mintWigzWaxTests(how_many_wigzwaxtests)
  ...
  return (
    <div id="bodyy" className="flex flex-col items-center justify-center min-h-screen py-2">
      <Head>
        ...
        <meta property="og:title" content="Wigz Wax Works" key="ogtitle" />
      ...
      <div >
          <div className="flex items-center justify-between w-full border-b-2	pb-6">
            ...
            <nav className="flex flex-wrap flex-row justify-around Poppitandfinchsans">
              ...
              <a href="/#traits" className="text-4xl text-white hover:text-black m-6">WaxTest traits</a>
        ...
        <div className="md:w-2/3 w-4/5">
          ...
          <div className="mt-6 border-b-2 py-6">
            ...
            <div className="flex flex-col items-center">
                ...
                <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none my-4 ">TOTAL WAXTESTS MINTED:  <span className="text-blau text-6xl"> {!signedIn ?  <>-</>  :  <>{totalSupply}</> } / 1000</span></span>

                <div id="mint" className="flex justify-around  mt-8 mx-6">
                  <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none px-3 font-bold">WIGZWAXTESTS</span>
                  
                  <input 
                                      type="number" 
                                      min="1"
                                      max="20"
                                      value={how_many_wigzwaxtests}
                                      onChange={ e => set_how_many_wigzwaxtests(e.target.value) }
                                      name="" 
                ...
                {saleStarted ? 
                <button onClick={() => mintWigzwaxtests (how_many_wigzwaxtests)} className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">MINT {how_many_wigzwaxtests} wigzwaxtests for {(wigzwaxtestsPrice * how_many_wigzwaxtests) / (10**18)} ETHEREUM </button>        
